...
def train(is_debug=False):
    ...
    for epoch in range(epoch_num):
        ...
        for i, batch in enumerate(getBatch(batch_size, index_train)):
            # 执行一个batch的训练
            _, loss, decoder_prediction, intent, mask, slot_W = model.step(sess, "train", batch, embeddings)
            # if i == 0:
            #     index = 0
        ...
        for j, batch in enumerate(getBatch(batch_size, index_test)):
            decoder_prediction, intent, intent_accuracy = model.step(sess, "test", batch, embeddings)
