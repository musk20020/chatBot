...
def train(is_debug=False):
    ...
    test_data = open( "dataset/redwine_training_data_musk.iob", "r" ).readlines()
    ...
    word2index, index2word, slot2index, index2slot, intent2index, index2intent = \
        get_info_from_training_data( test_data_ed )
    ...
    for epoch in range( epoch_num ):
        ...
        for i, batch in enumerate( getBatch( batch_size, index_train ) ):
            # 执行一个batch的训练
            _, loss, decoder_prediction, intent, mask, slot_W = model.step( sess, "train", batch, word_dic )
            # if i == 0:
            #     index = 0
        ...
        for j, batch in enumerate( getBatch( batch_size, index_test ) ):
            ...
            if j == 0:
                ...
                print ("Input Sentence        : ") + \
                    " ".join(batch[index][0][:sen_len])
                print ("Slot Truth            : ") + str(
                    index_seq2slot( batch[index][2], index2slot )[:sen_len] ).decode( 'string_escape' )
                print ("Slot Prediction       : ") + str(
                    index_seq2slot( decoder_prediction[index], index2slot )[:sen_len] ).decode( 'string_escape' )
