...
class Model:
    ...
    def build(self):
        ...
        if train:
            ...
            embed_npy_musk = np.load('embed_npy_musk_5491_6491.npy')
        ...
        #self.embeddings = embed_npy_musk.astype(np.float32)


        #embeddings_expend = np.zeros([3,64]).astype(np.float32)
        #embeddings_expend[0] = self.embeddings[4645]
        #embeddings_expend[1] = self.embeddings[4645]
        #embeddings_expend[2] = self.embeddings[4645]
        #self.embeddings = np.concatenate((self.embeddings,embeddings_expend))

        #np.save('embedding_Musk.npy',self.embeddings)
