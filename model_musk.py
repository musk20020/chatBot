...
class Model_musk:
    def __init__(self, input_steps, embedding_size, hidden_size, vocab_size, slot_size,
        ...
        self.intent_targets = tf.placeholder(tf.int32, [batch_size],
                                             name='intent_targets')
        self.embedding_npy_musk = tf.placeholder(tf.float32, [self.vocab_size, self.word_size])
    ...
    def build(self):
        ...
        self.actual_embedding = tf.add( tf.matmul( self.embeddings, self.weight ), self.bias )
    ...
    def step(self, sess, mode, trarin_batch, embedding):
        """ perform each batch"""
